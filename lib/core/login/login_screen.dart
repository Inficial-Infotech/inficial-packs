...
class _LoginScreenState extends State<LoginScreen> {
              ...
              child: state.step != LoginSteps.verifyOtp
                  ...
                  child: Column(
                    children: <Widget>[
                        ...
                        children: <Widget>[
                          if (state.step == LoginSteps.generateOtp)
                            PXBackButton(
                              ...
                              onPressed: () {
                                cubit.loginStepChanged(LoginSteps.chooseType);
                              },
                            )
                          else
                            Container(),
                          GestureDetector(
                            ...
                            child: const Icon(CupertinoIcons.clear, color: PXColor.black, size: 30),
                          ),
                        ],
                      ),
                      const SizedBox(height: PXSpacing.spacingXXL),
                      const Align(
                        ...
                        child: Text('Welcome back!', style: TextStyle(fontSize: PXFontSize.sizeXXL, fontWeight: FontWeight.bold)),
                      ),
                      ...
                      if (state.loginType == LoginType.phone)
                            ...
                            Row(
                              children: <Widget>[
                                GestureDetector(
                                  ...
                                  onTap: () {
                                    _showPicker(context, state.countries, state.country, (Country country) {
                        ...
                        Row(
                          children: <Widget>[
                            const Icon(CupertinoIcons.mail, color: PXColor.black),
                            const SizedBox(width: PXSpacing.spacingS),
                            Expanded(
                              child: CupertinoTextField(
                                decoration: BoxDecoration(border: Border.all(style: BorderStyle.none)),
                      ...
                      if (state.step == LoginSteps.chooseType)
                        Align(
                          ...
                          child: GestureDetector(
                            onTap: () {
                              cubit.loginTypeChanged(state.loginType == LoginType.phone ? LoginType.email : LoginType.phone);
                            },
                            child: Text('use ${state.loginType == LoginType.email ? 'phone' : 'email'} instead',
                            ...
                            child: Row(
                              ...
                              children: <Widget>[
                                Container(
                                  ...
                                  child: Padding(padding: const EdgeInsets.all(10), child: Image.asset('assets/images/mac.png')),
                                ),
                                const SizedBox(width: PXSpacing.spacingS),
                                GestureDetector(
                                  onTap: () async {
                      ...
                      if (state.step == LoginSteps.chooseType)
                        CupertinoButton.filled(
                            onPressed: state.loading
                              ...
                              if (state.loginType == LoginType.phone) {
                                final bool isPhoneExists = await cubit.isPhoneNumberExisting('${state.country.dialCode}${state.phone}');
                                if (isPhoneExists) {
                                  cubit.loginStepChanged(LoginSteps.generateOtp);
                                } else {
                                  showFailureSnackBar('Phone number is not registered');
                                }
                              } else {
                                final bool isEmailExists = await cubit.checkIfEmailIsExists();
                                if (!isEmailExists) {
                                  showFailureSnackBar('Email is not registered');
                                } else {
                                  cubit.loginStepChanged(LoginSteps.generateOtp);
                                }
                              }
                        ...
                        CupertinoButton.filled(
                          onPressed: state.loading
                            ...
                            if (state.loginType == LoginType.email) {
                              showSuccessSnackBar('A code has been sent to ${state.email}');
                ...
                child: Column(
                  children: <Widget>[
                      ...
                      children: <Widget>[
                        PXBackButton(
                          ...
                          onPressed: () {
                            if (state.step == LoginSteps.generateOtp) {
                              cubit.loginStepChanged(LoginSteps.chooseType);
                            } else if (state.step == LoginSteps.verifyOtp) {
                              cubit.loginStepChanged(LoginSteps.generateOtp);
                            }
                          },
                        ),
                    ...
                    Align(
                      alignment: Alignment.topLeft,
                      child: Text('${state.loginType == LoginType.email ? 'Email' : 'Phone'} Verification',
                          ...
                          Center(
                            child: CupertinoButton.filled(
                              onPressed: state.loading
                                  ? null
                                  : () async {
                                bool isEmpty = state.otp.any((String element) => element.isEmpty);
                                if (isEmpty) {
                                  return null;
                                }
                                cubit.setLoading(true);
                                if (state.loginType == LoginType.phone) {
                                  String? id =
                                  await context.read<LoginRepository>().verifyPhoneOtp(state.otp.join(), state.verificationId);
                                  if (id == null) {
                                    showFailureSnackBar('Invalid code');
                                  } else {
                                    Navigator.pop(context);
                                    navigateToSelectNameScreen(context);
                                  }
  ...
  Future<void> verifyPhoneNumber(String phoneNumber, BuildContext context, LoginState state, LoginCubit cubit) async {
    try {
      await _auth.verifyPhoneNumber(
          ...
          codeSent: (String code, int? i) {
            cubit.setLoading(false);
            showSuccessSnackBar('A code has been sent to ${state.country.dialCode}${state.phone}');
...
void showFailureSnackBar(String message) {
  Fluttertoast.showToast(msg: message, gravity: ToastGravity.BOTTOM, backgroundColor: PXColor.red, textColor: PXColor.white);
}
